...
namespace UIBusReservationSystem.Controllers
{
    public class AuthUserUIController : Controller
    {
        ...
        [HttpPost]
        public IActionResult ResetPass(UserReg user, int i=0)
        {
        ...
        public IActionResult Cancel(int bookId)
        {
            ...
            bool remseat = Helper.RemoveSeats(seats);
            schedule.SeatAvl += seats.Count;
            bool updated = Helper.UpdateSeatAvailability(schedule, schedule.ScheduleId);
            ADJ adj = new();
            adj.user = user;
            adj.amt = (decimal)booking.Tcharge;
            bool added = Helper.AddMoneyToWallet(adj);
            bool remPay = Helper.RemovePayment(payment.PaymentId);
            bool removed = Helper.RemoveBooking(bookId);
            UserReg User = Helper.GetUserById(user.UserEmail);
